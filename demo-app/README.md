...
Then open `http://localhost:3000` in Chrome with HydraLens installed, or point the CLI at it:
...
The Playwright e2e and system tests spin up this app via the `webServer` option in
`playwright.config.ts` and `packages/extension/playwright.config.ts`. Port 3000 is
used by the root config (system tests); port 3001 by the extension config.
