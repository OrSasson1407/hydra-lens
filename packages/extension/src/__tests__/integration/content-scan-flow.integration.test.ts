...
function makeChromeMock() {
  ...
  return {
    runtime: {
      sendMessage: vi.fn().mockImplementation((msg: ChromeMessage) => {
...
describe("content-scan-flow.integration", () => {
  ...
  it("successful fetch ? HYDRALENS_RESULTS message sent", async () => {
    ...
    await chrome.runtime.sendMessage({
      ...
      payload: { mismatches, totalFound: 1 },
    });
    expect(chrome.sentMessages).toContainEqual(
      expect.objectContaining({ type: "HYDRALENS_RESULTS" })
    );
  });
  ...
  it("AbortError ? HYDRALENS_ERROR sent with timeout message", async () => {
    ...
    expect(chrome.sentMessages).toContainEqual(
      expect.objectContaining({ type: "HYDRALENS_ERROR" })
    );
  });
  ...
  it("no overlay drawn when querySelector returns null", () => {
    ...
    const drawn: string[] = [];
    mismatches.forEach((m) => {
      const clientEl: HTMLElement | null = null;
      if (clientEl) drawn.push(m.selector);
    });
