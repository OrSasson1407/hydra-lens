...
function drawOverlay(element: HTMLElement, mismatch: Mismatch): void {
  ...
  // Row 1 — severity badge + component name
  ...
  severityBadge.style.cssText = `
    ...
    color: ${style.border}; text-transform: uppercase;
  `;
  ...
  serverSpan.textContent = mismatch.serverText.slice(0, 50);
  ...
  clientSpan.textContent = mismatch.clientText.slice(0, 50);
